    ...
    ((), eth0Tx) = toSignals circ (eth0_rx,())
      where
        ...
        circ = do
          ...
          let (rxRGMII, txRGMII) = unsafeRgmiiCircuits eth0Txclk resetGen enableGen (delayg d80) (delayg d0) (\a b _ -> iddrx1f a b) (\a b _ -> oddrx1f a b)
    ...
    ((), eth1Tx) = toSignals circ (eth1_rx,())
      where
        ...
        circ = do
          ...
          let (rxRGMII, txRGMII) = unsafeRgmiiCircuits eth1Txclk resetGen enableGen (delayg d80) (delayg d0) (\a b _ -> iddrx1f a b) (\a b _ -> oddrx1f a b)
