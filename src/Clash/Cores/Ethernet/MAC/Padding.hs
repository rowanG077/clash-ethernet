...
data State = LongEnough | TooShort (Index 47) | AddPadding (Index 47)
...
payloadPadder :: HiddenClockResetEnable dom => Circuit (SingleByteStream dom) (SingleByteStream dom)
payloadPadder = mealyToCircuit machineAsFunction initialState where
  ...
  machineAsFunction (AddPadding n) (_, recvACK) = (newState, (notReady, Just nullByte))
    where
      newState
        -- No transfer happening this clock cycle
        | not $ _tready recvACK = AddPadding n
        -- Successfully padded until the minimum length
        | n + 1 >= 46 = initialState
        -- Keep padding null bytes
        | n + 1 < 46 = AddPadding (n+1)
  ...
  machineAsFunction (TooShort n) (Just inp, recvACK) = (newState, (recvACK, output))
    where
      ...
      newState
        ...
        | newCount < 46 && _tlast inp = AddPadding newCount
